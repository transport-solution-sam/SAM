	...
	// Pass this if window is not defined yet
		...
		(
				...
				function(window, noGlobal) {
					 ...
					 * 
					 * Copyright 2013 jQuery Foundation, Inc. and other contributors Released under the MIT license
					 * http://jquery.org/license
					 * 
					 * Date: 2014-01-13
					 */
					(function(window) {
						...
						setDocument = Sizzle.setDocument = function(node) {
							...
							// Support: IE<8
							// Verify that getAttribute really returns attributes and not properties (excepting IE8
							// booleans)
						...
						Expr = Sizzle.selectors = {
							...
							preFilter: {
								...
								"CHILD": function(match) {
									/*
									 * matches from matchExpr["CHILD"] 1 type (only|nth|...) 2 what (child|of-type) 3
									 * argument (even|odd|\d*|\d*n([+-]\d+)?|...) 4 xn-component of xn+y argument
									 * ([+-]?\d*n|) 5 sign of xn-component 6 x of xn-component 7 sign of y-component 8 y
									 * of y-component
									 */
								...
								"ATTR": function(name, operator, check) {
									return function(elem) {
										...
										return operator === "=" ? result === check
												: operator === "!=" ? result !== check : operator === "^=" ? check
								...
								if (xml) {
									...
									while ((elem = elem[dir])) {
										if (elem.nodeType === 1 || checkNonElements) {
											...
											if ((oldCache = outerCache[dir]) && oldCache[0] === dirruns
													&& oldCache[1] === doneName) {
					 ...
					 * 
					 * options: an optional list of space-separated options that will change how the callback list
					 * behaves or a more traditional option object
					 * 
					 * By default a callback list will act like an event callback list and can be "fired" multiple
					 * times.
					 * 
					 * Possible options:
					 * 
					 ...
					 * 
					 * memory: will keep track of previous values and will call any callback added after the list has
					 * been fired right away with the latest "memorized" values (like a Deferred)
					 * 
														...
														jQuery.each(tuples, function(i, tuple) {
															...
															deferred[tuple[1]](function() {
																...
																if (returned && jQuery.isFunction(returned.promise)) {
																	returned.promise().done(newDefer.resolve).fail(
																			newDefer.reject).progress(newDefer.notify);
																} else {
					 ...
					 * 
					 * 1. Enforce API surface and semantic compatibility with 1.9.x branch 2. Improve the module's
					 * maintainability by reducing the storage paths to a single mechanism. 3. Use the same single
					 * mechanism to support "private" and "user" data. 4. _Never_ expose "private" data to user code
					 * (TODO: Drop _data, _removeData) 5. Avoid exposing implementation details on user objects (eg.
					 * expando properties) 6. Provide a clear path for implementation upgrade to WeakMap in 2014
					 */
					...
					jQuery
							.extend({
								...
								// not intended for public consumption - generates a queueHooks object, or returns the
								// current one
					...
					(function() {
						var fragment = document.createDocumentFragment(), div = fragment.appendChild(document
								.createElement("div"));
					 ...
					 * addEvent library for many of the ideas.
					 */
					jQuery.event = {
						...
						add: function(elem, types, handler, data, selector) {
							...
							if (!(eventHandle = elemData.handle)) {
								eventHandle = elemData.handle = function(e) {
									...
									return typeof jQuery !== strundefined && jQuery.event.triggered !== e.type ? jQuery.event.dispatch
						...
						trigger: function(event, data, elem, onlyHandlers) {
							...
							var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem || document ], type = hasOwn
						...
						dispatch: function(event) {
							...
							var i, j, ret, matched, handleObj, handlerQueue = [], args = slice.call(arguments), handlers = (data_priv
							...
							while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
								...
								while ((handleObj = matched.handlers[j++]) && !event.isImmediatePropagationStopped()) {
									...
									// 2) have namespace(s) a subset or equal to those in the bound event (both can have
									// no namespace).
					...
					jQuery.Event = function(src, props) {
						...
						if (src && src.type) {
							...
							// Support: Android < 4.0
							src.defaultPrevented === undefined && src.getPreventDefault && src.getPreventDefault() ? returnTrue
									: returnFalse;
					...
					jQuery
							.extend({
								clone: function(elem, dataAndEvents, deepDataAndEvents) {
									var i, l, srcElements, destElements, clone = elem.cloneNode(true), inPage = jQuery
					...
					jQuery.fn
							.extend({
								...
								domManip: function(args, callback) {
									...
									if (isFunction
											|| (l > 1 && typeof value === "string" && !support.checkClone && rchecked
													.test(value))) {
									...
									if (l) {
										...
										if (first) {
											...
											// Use the original fragment for the last item instead of the first because
											// it can end up
					...
					function curCSS(elem, name, computed) {
						...
						if (computed) {
							...
							// iOS < 6 (at least) returns percentage for a larger set of values, but width seems to be
							// reliably pixels
					...
					(function() {
						...
						divReset = "padding:0;margin:0;border:0;display:block;-webkit-box-sizing:content-box;"
								+ "-moz-box-sizing:content-box;box-sizing:content-box", docElem = document.documentElement, container = document
					...
					rdisplayswap = /^(none|table(?!-c[ea]).+)/, rnumsplit = new RegExp("^(" + pnum + ")(.*)$", "i"), rrelNum = new RegExp(
							"^([+-])=(" + pnum + ")", "i"),
					...
					function getWidthOrHeight(elem, name, extra) {
						...
						var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth : elem.offsetHeight, styles = getStyles(elem), isBorderBox = jQuery
							...
							if (show) {
								...
								if (!values[index]) {
									...
									if (display && display !== "none" || !hidden) {
										data_priv.set(elem, "olddisplay", hidden ? display : jQuery
												.css(elem, "display"));
									}
								}
							}
					...
					jQuery.extend({
						...
						style: function(elem, name, value, extra) {
							...
							if (value !== undefined) {
								...
								// but it would mean to define eight (for every problematic property) identical
								// functions
								...
								jQuery.cssHooks[name] = {
									get: function(elem, computed, extra) {
										if (computed) {
											...
											// however, it must have a current display style that would benefit from
											// this
											return elem.offsetWidth === 0
													&& rdisplayswap.test(jQuery.css(elem, "display")) ? jQuery.swap(
									...
									set: function(elem, value, extra) {
										...
										return setPositiveNumber(elem, value, extra ? augmentWidthOrHeight(elem, name,
												extra, jQuery.css(elem, "boxSizing", false, styles) === "border-box",
												styles) : 0);
									}
								};
							});
					...
					var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp("^(?:([+-])=|)("
						...
						"*": [ function(prop, value) {
							var tween = this.createTween(prop, value), target = tween.cur(), parts = rfxnum.exec(value), unit = parts
									&& parts[3] || (jQuery.cssNumber[prop] ? "" : "px"),
							...
							if (start && start[3] !== unit) {
								...
								do {
									...
									// Use a string for doubling factor so we don't accidentally see scale as unchanged
									// below
									scale = scale || ".5";
									...
									// And breaking the loop if scale is unchanged or perfect, or if we've just had
									// enough
					...
					function defaultPrefilter(elem, props, opts) {
						...
						var prop, value, toggle, tween, hooks, oldfire, display, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
					...
					jQuery.fn
							.extend({
								...
								stop: function(type, clearQueue, gotoEnd) {
									...
									return this
											.each(function() {
												var dequeue = true, index = type != null && type + "queueHooks", timers = jQuery.timers, data = data_priv
														.get(this);
								...
								finish: function(type) {
									...
									return this
											.each(function() {
												var index, data = data_priv.get(this), queue = data[type + "queue"], hooks = data[type
							...
							.extend({
								...
								prop: function(elem, name, value) {
									...
									if (value !== undefined) {
										...
										return hooks && "get" in hooks && (ret = hooks.get(elem, name)) !== null ? ret
												: elem[name];
									}
								},
							...
							.extend({
								...
								removeClass: function(value) {
									var classes, elem, cur, clazz, j, finalValue, proceed = arguments.length === 0
											|| typeof value === "string" && value, i = 0, len = this.length;
					 ...
					 * These are called: - BEFORE asking for a transport - AFTER param serialization (s.data is a string
					 * if s.processData is true) 3) key is the dataType 4) the catchall symbol "*" can be used 5)
					 ...
					 * will start with transport dataType and THEN go to "*" if needed
					 */
					transports = {},
					...
					/*
					 * Handles responses to an ajax request: - finds the right dataType (mediates between content-type
					...
					/*
					 * Chain conversions given the request and the original response Also sets the responseXXX fields on
					 * the jqXHR instance
					 */
					...
					jQuery
							.extend({
								...
								ajaxSettings: {
									...
									/*
									 * timeout: 0, data: null, dataType: null, username: null, password: null, cache:
									 * null, throws: false, traditional: false, headers: {},
									 */
						...
						if (jQuery.isArray(obj)) {
							...
							jQuery.each(obj, function(i, v) {
								if (traditional || rbracket.test(prefix)) {
									...
									buildParams(prefix + "[" + (typeof v === "object" ? i : "") + "]", v, traditional,
											add);
								}
							});
					...
					// data: string of html
					// context (optional): If specified, the fragment will be created in this context, defaults to
					// document
					...
					jQuery.fn.extend({
						...
						position: function() {
							...
							// Fixed elements are offset from window (parentOffset = {top:0, left: 0}, because it is its
							// only offset parent
